...
\subsubsection{Prueba 4: Superación de la Degradación Temporal (Variación de Período Semilla)}

En el \textit{baseline}, comprobamos que la alteración estática del período de los datos semilla (\textit{Seed Data}) generaba una curva de degradación temporal inevitable (el fallo en horizontes de medio-corto plazo, o $\Delta 2$, por extrapolación lineal). 

Para dar continuidad a esta línea investigativa en la red Multi-Agente, abandonamos el enfoque de \textit{cutoff} estático. En lugar de inicializar la simulación con ventanas semilla rígidamente separadas, alimentamos a los agentes con flujos de datos iterativos que simulaban el paso del tiempo real. Los resultados demostraron que el ecosistema heterogéneo —al estar sometido a fricción dialéctica constante— procesa las actualizaciones temporales (cambios de período) sin caer en la extrapolación lineal ciega que afectaba al modelo individual. La diversidad arquitectónica actuó como un amortiguador predictivo, estabilizando la inferencia a corto y medio plazo siempre y cuando el flujo de inyección no se interrumpiera.
