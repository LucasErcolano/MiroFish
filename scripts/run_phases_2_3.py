...
def main():
    ...
    ladder_models = [m for m in config["model_ladder"] if m != primary_model]

    print("=== MiroFish S2 PHASES 2 & 3: Model Ladder & Robustness ===")
    
    # We use R80-D2 as the Optimal Condition
    target_cond = next(c for c in config["line5_conditions"] if c["id"] == "R80-D2")

    tasks = []
    
    # Phase 2: Model Ladder
    for m in ladder_models:
        tasks.append((m, target_cond, None)) # None means standard folder
        
    # Phase 3: 3 Replicas for primary model
    for i in range(1, 4):
        tasks.append((primary_model, target_cond, f"rep{i}"))

    sim_count = 0
    total_sims = len(tasks)

    for model_full_name, cond, rep in tasks:
        sim_count += 1
        cond_id = cond["id"]
        display_name = model_full_name.split("/")[-1]
        
        folder_suffix = f"_{rep}" if rep else ""
        output_dir = base_dir / f"{cond_id}_{display_name}{folder_suffix}"
        
        if output_dir.exists() and (output_dir / "verdict.json").exists():
            print(f"Skipping {output_dir.name} (exists).")
            continue

        print(f"\n[{sim_count}/{total_sims}] PROCESSING: {display_name} @ {cond_id} {folder_suffix}")

        # Routing
        if "openrouter/" in model_full_name:
            api_key = os.environ.get("OPENROUTER_API_KEY")
            base_url = "https://openrouter.ai/api/v1"
            actual_model = model_full_name.replace("openrouter/", "")
        elif "deepinfra/" in model_full_name:
            api_key = os.environ.get("DEEPINFRA_API_KEY")
            base_url = "https://api.deepinfra.com/v1/openai"
            actual_model = model_full_name.replace("deepinfra/", "")
        else:
            api_key = os.environ.get("DEEPINFRA_API_KEY")
            base_url = "https://api.deepinfra.com/v1/openai"
            actual_model = model_full_name

        os.environ.update({"LLM_API_KEY": api_key, "LLM_BASE_URL": base_url, "LLM_MODEL_NAME": actual_model, "OPENAI_API_KEY": api_key, "OPENAI_BASE_URL": base_url})
        Config.LLM_API_KEY, Config.LLM_BASE_URL, Config.LLM_MODEL_NAME = api_key, base_url, actual_model
    ...
    print("\n[COMPLETED] Phases 2 and 3 finished.")
