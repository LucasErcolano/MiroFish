...
\paragraph{Ruteo y observabilidad.} Por defecto, todos los agentes de una simulación corren sobre un mismo LLM. Implementamos un enrutador (\texttt{model\_router.py}) que asigna modelos distintos mediante un mapa declarativo en YAML, junto con telemetría por llamada (\texttt{llm\_telemetry.py}) para registrar modelo, tokens, latencia, costo y errores. El \textit{Observability Dock} reúne esos datos con los artefactos de cada ejecución, las trazas de \textit{Deep Search} y la memoria Wiki, de modo que una conclusión pueda rastrearse hasta su configuración y evidencia.

\paragraph{Grafo, deduplicación y memoria Wiki.} El grafo se persiste en Neo4j y conserva entidades, episodios y relaciones. Para evitar redundancia residual, la etapa posterior a la ingesta compara nodos aislados contra los ya conectados mediante \textit{embeddings}: si la similitud coseno supera 0.85, el nodo aislado se descarta como redundante. No se trata de una fusión global de entidades. En paralelo, una memoria Wiki opcional compila fuentes, afirmaciones, cronología y contradicciones para el agente de reporte; funciona como artefacto auditable y no como reemplazo del grafo operativo.
...
\subsubsection{Wiki y observabilidad de la ejecución}

La memoria Wiki y el \textit{Observability Dock} no se evalúan como predictores separados: su aporte es de ingeniería experimental. La Wiki compila determinísticamente afirmaciones, fuentes, cronología y contradicciones dentro de un presupuesto de contexto; el panel expone la configuración, los artefactos, el ruteo, tokens, costo, latencia y errores. Así se separa el \textit{input} del \textit{ground truth} y se puede auditar qué evidencia y qué modelo sostuvieron cada reporte, incluidos los resultados negativos de Deep Search.
...
\subsubsection{Ruido financiero en alta profundidad}

Esta prueba, independiente de T0--T3, introdujo información no predictiva y varió su momento de entrada. Bajo alta profundidad ($R=80$, $D=2$), el modelo asimiló especulación financiera (\textit{herd behavior}) y, en esa variante aislada, alcanzó un MAE de 0.9875\% al anclar un escenario recesivo. La coincidencia no convierte al ruido en evidencia útil: muestra vulnerabilidad a estímulos salientes y al momento de inyección.

\subsubsection{Inyecciones programadas en los tres dominios (S3)}

Extendimos la inyección programada a Copa América, Bolivia e IPC para separar contenido, momento y cercanía temática. Cruzamos Gemma y Llama con control; señal favorable temprana, media y tardía; contraseñal media; ruido cercano; y ruido ajeno. Las 42 corridas fueron válidas: los seis controles dispararon cero eventos y las 36 condiciones inyectadas, exactamente uno (Tabla~\ref{tab:s3_cross_topic}).
