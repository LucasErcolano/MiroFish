    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <div class="prep-observability">
            <div class="prep-observability-header">
              <div>
                <span class="prep-title">{{ t('observability.preparationTrace.title') }}</span>
                <p class="prep-subtitle">{{ t('observability.preparationTrace.subtitle') }}</p>
              </div>
              <div class="prep-actions">
                ...
                <button
                  v-if="telemetryAvailable"
                  type="button"
                  class="prep-wiki-btn secondary"
                  @click="openTelemetry"
                >
                  {{ t('observability.openTelemetry') }}
                </button>
                <span class="prep-progress mono">{{ prepareProgress }}%</span>
              </div>
            </div>
            <EvidenceStageRail :stages="preparationTraceStages" compact>
              <template #research>
                <div class="deep-research-preview">
                  <div class="deep-research-source">
                    <span>{{ deepResearchSource }}</span>
                    <strong v-if="deepSearch?.size">{{ formatBytes(deepSearch.size) }}</strong>
                  </div>
                  <div class="deep-research-log-list">
                    <span v-for="item in deepResearchLogItems" :key="item">{{ item }}</span>
                  </div>
                  <div v-if="deepResearchFindings.length" class="deep-research-section-label">
                    <span>New context found</span>
                    <strong>{{ deepResearchFindings.length }} signals</strong>
                  </div>
                  <ul v-if="deepResearchFindings.length" class="deep-research-findings">
                    <li v-for="(finding, index) in deepResearchFindings" :key="`${index}-${finding}`">{{ finding }}</li>
                  </ul>
                  <div v-else class="deep-research-log">
                    <span>{{ progressMessage || 'Waiting for research trace...' }}</span>
                  </div>
                </div>
              </template>
              ...
              <template #profiles>
                <div v-if="profiles.length" class="prep-profile-chips">
                  <span v-for="profile in profiles.slice(0, 5)" :key="profile.user_id || profile.username || profile.name">
                    <strong>{{ profile.username || profile.name || 'Agent' }}</strong>
                    <small>{{ profile.profession || profile.entity_type || 'Profile' }}</small>
                  </span>
                </div>
              </template>

              <template #config>
                <div v-if="simulationConfig" class="prep-config-preview">
                  <span v-if="autoGeneratedRounds"><strong>{{ autoGeneratedRounds }}</strong> rounds</span>
                  <span v-if="simulationConfig.event_config?.initial_posts?.length">
                    <strong>{{ simulationConfig.event_config.initial_posts.length }}</strong> initial posts
                  </span>
                  <span v-if="simulationConfig.twitter_config">Plaza</span>
                  <span v-if="simulationConfig.reddit_config">Community</span>
                </div>
              </template>
            </EvidenceStageRail>
          </div>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              <span class="stat-value">{{ profileExpectedTotal || '-' }}</span>
              <span class="stat-label">Final agents</span>
            </div>
            <div class="stat-card" v-if="candidateTotal">
              <span class="stat-value">{{ candidateTotal }}</span>
              <span class="stat-label">Graph candidates</span>
            </div>
            <div class="stat-card">
...
import { buildPreparationMilestones, normalizeWikiPages } from '../composables/useObservabilitySummary'
...
const candidateTotal = ref(null)
const profileExpectedTotal = ref(null)
...
const preparationMilestones = computed(() => buildPreparationMilestones({
  stageKey: currentStageKey.value,
  stageName: currentStage.value,
  profileCount: profiles.value.length,
  expectedTotal: profileExpectedTotal.value,
  candidateTotal: candidateTotal.value,
  profiles: profiles.value,
  simulationConfig: simulationConfig.value,
  hasConfig: Boolean(simulationConfig.value),
  wiki: wiki.value,
  deepSearch: deepSearch.value,
  deduplication: deduplication.value
}))
const preparationTraceStages = computed(() => preparationMilestones.value.map(item => ({
  key: item.key,
  label: item.label,
  ...
  meta: item.meta || [],
  defaultOpen: item.status === 'active' || item.status === 'warning' || (item.key === 'wiki' && Boolean(wikiStats.value))
})))
...
const telemetryAvailable = computed(() => Boolean(manifest.value?.telemetry || telemetry.value?.records_count))

const cleanResearchLine = (line) => {
  return String(line || '')
    .replace(/^[-*#>\d.\s]+/, '')
    .replace(/\*\*/g, '')
    .replace(/\s+/g, ' ')
...
const extractResearchBody = (content) => {
...
const deepResearchSource = computed(() => {
  ...
  if (!content) {
    return currentStageKey.value === 'research' ? 'Collecting external context' : 'Research trace pending'
  }
  if (/TAVILY GROUNDED/i.test(content)) return 'Tavily grounded research'
  if (/LLM INTERNAL KNOWLEDGE/i.test(content)) return 'LLM fallback research'
  if (/Deep Search failed/i.test(content)) return 'Research trace failed'
  return 'Research trace saved'
})

const deepResearchFindings = computed(() => {
  const content = extractResearchBody(deepSearch.value?.content || '')
  if (!content) return []

  return content
    .split(/\n+/)
    .map(cleanResearchLine)
    .filter(line => {
...
const deepResearchLogItems = computed(() => {
  if (deepSearch.value?.available) {
    return [
      'Research trace saved',
      deepResearchSource.value,
      deepSearch.value.modified_at ? `Updated ${new Date(deepSearch.value.modified_at * 1000).toLocaleTimeString()}` : 'Ready for Wiki and profiles'
    ]
  }

  if (currentStageKey.value === 'research') {
    return [
      'Research task started',
      progressMessage.value || 'Collecting market context'
    ]
  }

  return ['Waiting for Deep Research stage']
})
...
const openWiki = () => {
  if (!props.simulationId || !manifest.value?.wiki) return
  showWikiSlideOver.value = true
}

const openTelemetry = () => {
  if (!props.simulationId || !telemetryAvailable.value) return
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        ...
        candidateTotal.value = res.data.prepare_info?.candidate_entities_count || null
        profileExpectedTotal.value = res.data.prepare_info?.deduped_entities_count || res.data.prepare_info?.entities_count || null
      ...
      // Prepare returns pre-Dedup graph candidates first. The final profile
      // total is set later from Dedup/progress_detail or profiles/realtime.
      if (res.data.candidate_entities_count || res.data.expected_entities_count) {
        candidateTotal.value = res.data.candidate_entities_count || res.data.expected_entities_count
        addLog(t('log.zepEntitiesFound', { count: candidateTotal.value }))
        if (res.data.entity_types && res.data.entity_types.length > 0) {
          addLog(t('log.entityTypes', { types: res.data.entity_types.join(', ') }))
        }
      }
      if (res.data.deduped_entities_count) {
        profileExpectedTotal.value = res.data.deduped_entities_count
      }
...
const refreshObservabilityArtifacts = async () => {
  if (!props.simulationId) return
  try {
    await loadArtifacts()
  } catch (err) {
    console.warn('获取 observability artifacts 失败:', err)
  }
}

const startArtifactsPolling = () => {
  if (artifactsTimer) return
  refreshObservabilityArtifacts()
  artifactsTimer = setInterval(refreshObservabilityArtifacts, 4000)
}

const stopArtifactsPolling = () => {
  if (artifactsTimer) {
    ...
    if (res.success && res.data) {
      ...
      progressMessage.value = data.message || ''
	      ...
	      if (data.progress_detail) {
	        currentStageKey.value = data.progress_detail.current_stage || ''
	        currentStage.value = data.progress_detail.current_stage_name || ''
	        if (data.progress_detail.candidate_entities_count) {
	          candidateTotal.value = data.progress_detail.candidate_entities_count
	        }
	        if (data.progress_detail.deduped_entities_count || data.progress_detail.current_stage === 'generating_profiles') {
	          profileExpectedTotal.value = data.progress_detail.deduped_entities_count || data.progress_detail.total_items || profileExpectedTotal.value
	        }
	        if (data.progress_detail.current_stage === 'generating_profiles' && !profilesTimer) {
        ...
        if (match) {
          ...
          currentStageKey.value = match[3].trim()
        }
        // 输出消息日志（避免重复）
      ...
      if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
        ...
        stopPolling()
        stopProfilesPolling()
        await refreshObservabilityArtifacts()
        await loadPreparedData()
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
	      ...
	      if (res.data.candidate_entities_count) {
	        candidateTotal.value = res.data.candidate_entities_count
	      }
	      if (res.data.deduped_entities_count || res.data.total_expected) {
	        profileExpectedTotal.value = res.data.deduped_entities_count || res.data.total_expected
	      }
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
	        ...
	        const total = profileExpectedTotal.value || '?'
        ...
        addLog(t('log.agentProfile', { current: currentCount, total: total, name: profileName, profession: latestProfile?.profession || t('step2.unknownProfession') }))
	        ...
	        // 如果全部生成完成
	        if (profileExpectedTotal.value && currentCount >= profileExpectedTotal.value) {
...
.prep-wiki-btn.secondary {
  background: #FFFFFF;
  color: #0F172A;
}

.prep-wiki-btn.secondary:hover {
  background: #F8FAFC;
}
...
.deep-research-log-list {
  display: flex;
  flex-wrap: wrap;
  gap: 6px;
}

.deep-research-log-list span {
  max-width: 100%;
  padding: 5px 8px;
  border: 1px solid #E2E8F0;
  border-radius: 999px;
...
.deep-research-section-label strong {
  flex-shrink: 0;
  color: #64748B;
  font-family: 'JetBrains Mono', monospace;
  font-size: 10px;
...
.prep-profile-chips span,
.prep-config-preview span {
  min-width: 0;
  padding: 7px 8px;
  border: 1px solid #E2E8F0;
  border-radius: 7px;
